...
from unmute.report_generator import generate_html
...
async def generate_report(request: TranscriptRequest):
    ...
    try:
        ...
        print(f"DEBUG: MISTRAL_API_KEY present: {bool(api_key)}") # DEBUG PRINT
        if not api_key:
             print("DEBUG: MISTRAL_API_KEY is missing!")
             raise HTTPException(status_code=500, detail="MISTRAL_API_KEY not set")
             
        client = Mistral(api_key=api_key)

        print("DEBUG: Sending request to Mistral...")
        ...
        print("DEBUG: Mistral response received.")

        # Extract content
        raw_output = response.choices[0].message.content
        print(f"DEBUG: Raw output: {raw_output[:100]}...")
        ...
        try:
            ...
            # Manually inject the transcript back into the result so the frontend receives it
        ...
        print(f"DEBUG: Report Generation Exception: {e}") # DEBUG PRINT
...
async def generate_report_html(request: TranscriptRequest):
    """
    Generates an HTML version of the report directly.
    """
    # Reuse the JSON generation logic
    # Note: In a real app we might refactor the logic out to share it cleaner,
    # but calling the handler directly or duplicating the internal logic is fine for now.
    # To keep it simple and avoid refactoring everything, I'll just call the logic helper if I extract it,
    # or just duplicate the few lines of calling Mistral.
    # Actually, let's just refactor slightly to avoid duplication if possible.
    # But for "fast and robust", I will just call generate_report directly!
    
    json_report = await generate_report(request)
    html_content = generate_html(json_report)
