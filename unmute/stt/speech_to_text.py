...
class SpeechToText(ServiceWithStartup):
    def __init__(
        self, base_url: BaseUrl = GRADIUM_BASE_URL, delay_sec: float = STT_DELAY_SEC
    ):
    ...
    async def send_audio(self, audio: np.ndarray) -> None:
        ...
        assert audio.dtype in (np.int16, np.float32), f"Unexpected dtype {audio.dtype}"
        ...
        base64_audio = base64.b64encode(audio.tobytes()).decode("ascii")

        await self._send({"type": "audio", "audio": base64_audio})
    ...
    async def start_up(self):
        logger.info(f"Connecting to STT {self.base_url}...")
        self.websocket = await websockets.connect(
            self.base_url.ws_url(SPEECH_TO_TEXT_PATH), additional_headers=HEADERS
        )
        logger.info("Connected to STT, sending setup message")
        setup_message = {
            "type": "setup",
            "input_format": "pcm",
            "model_name": "default",
        }
        await self._send(setup_message)
    ...
    async def shutdown(self):
        ...
        if self.time_since_first_audio_sent.started:
            ...
            mt.STT_AUDIO_DURATION.observe(self.sent_samples / STT_SAMPLE_RATE)
    ...
    async def __aiter__(
        ...
        try:
            async for message_bytes in self.websocket:
                data = json.loads(message_bytes)  # type: ignore
                    ...
                    case STTStepMessage():
                        ...
                        if n_steps_to_wait > 0:
                            ...
                            self.pause_prediction.update(
                                dt=FRAME_TIME_SEC,
                                new_value=message.vad[2].inactivity_prob,
                            )
